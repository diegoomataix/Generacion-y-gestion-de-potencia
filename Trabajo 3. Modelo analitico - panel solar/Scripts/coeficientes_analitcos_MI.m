...
I_modelo =zeros(8,max(N_exp));
str = ["RTC_error_PbP","TNJ_error_PbP","ZTJ_error_PbP","G30C_error_PbP","PWP_error_PbP","KC2_error_PbP","SPV_error_PbP","PSC_error_PbP"];
for i = 1:size(dat,2) % bucle para los distintos paneles
    N = N_exp(i);
...
error(i) =  RECT(I_modelo(i,1:N),I_exp);
RMSE_adim(i) = RMSE_NonDim(I_modelo(i,1:N),I_exp);
...
if choose == 1
    rs0 = [0.1 0.29817 0.323727530 0.05 2.25 -0.5 -0.5 0.01];
elseif choose == 2
    rs0 = [1 -8.58 0.07];
end
