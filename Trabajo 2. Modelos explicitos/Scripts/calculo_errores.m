...
for model = 1:3
   ...
   if size(u,1) > 1
    ...
    [RMSE_PbP_Dim(:),RMSE_PbP_NonDim(:), I_model(:)] = error_PbPfun(u,V,I_exp);
...
%     hold on
if model == 1
    plot(V(:), RMSE_PbP_NonDim(:),'-sk','LineWidth',2, 'MarkerSize', 12,'MarkerIndices',1:50:length(V_plot))
elseif model == 2 
       plot(V(:), RMSE_PbP_NonDim(:),'-ok','LineWidth',2,'MarkerSize', 12,'MarkerIndices',1:50:length(V_plot))
elseif model == 3
        plot(V(:), RMSE_PbP_NonDim(:),'-dk','LineWidth',2,'MarkerSize', 12,'MarkerIndices',1:50:length(V_plot))
end 
